...
int main(int argc, char** argv)
{
	...
	/** ......... PARSING STAGE! .............
	LOAD UP ALL THE USER INFO
	Get all the information from the User's input Parsed
	**/
	...
	//Parse projector details from the images list 
	...
	//Put projector params into the Graycode functions
	params.width = stoi(projW[0]);
	params.height = stoi(projH[0]);
	double dFOV = ::atof(diagFOV[0].c_str());

	//Check if we screwed something up and there is missing info
	...
	//TODO update the parsing so it is not count based
	if (argc == 7)
	{
	...
	// LOAD CALIBRATION DATA ABOUT BOTH CAMERAS
	...
	Mat camAintrinsics, camAdistCoeffs, camBintrinsics, camBdistCoeffs, R, T; // Individual Camera Params
	Mat R1, R2, P1, P2, Q; // Stereo Calibration Params  
	...
	//LOADING ALL IMAGES
	//LOADING ALL THE IMAGES INTO MATs
	...
	//It also gets used throughout this code as the standard image for the whole array

	if (swapCameras) {
		cout << "NOTE CAMERA SWAP ON COLOR REFERENCE IMAGE" << endl;
	...
	for (size_t i = 0; i < numberOfPatternImages; i++)
	{
		...
		if (swapCameras) {
			cout << "NOTE CAMERA SWAP" << endl;

			captured_pattern[1][i] = imread(imagelist[i], IMREAD_GRAYSCALE); //Read Camera B in as grayscale
			captured_pattern[0][i] = imread(imagelist[i + numberOfPatternImages + 2], IMREAD_GRAYSCALE); //Read Camera A in as grayscale
		}
		else {
	...
	*/
	...
	if (rectify) {
		...
		*/
		...
		// Apply the Remapping to the images
		...
		//Rectify the white images from both cams
		...
		//Rectify the black images from both cams
